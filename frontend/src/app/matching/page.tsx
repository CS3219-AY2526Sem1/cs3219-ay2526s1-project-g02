...
export default function MatchingPage() {
    ...
    const [notification, setNotification] = useState<{ message: string; status: MatchStatus } | null>(null);
    ...
    const dismissNotification = () => {
        ...
        if (status === 'REQUEST_EXPIRED' || status === 'CANCELLED' || status === 'ERROR') {
            setStatus('IDLE');
        }
    };
    ...
    const [findMatch, { loading, error }] = useMutation(FIND_MATCH_MUTATION, {
        onCompleted(data) {
            ...
            if (result.matchFound) {
                ...
                setNotification({ message: 'Match found successfully!', status: 'MATCH_FOUND' });
        ...
        onError(err) {
            ...
            setStatus('ERROR');
            setNotification({ message: 'Failed to start match search. Please try again.', status: 'ERROR' });
        }
    })
    ...
    const [cancelMatch, { loading: canceling, error: cancelError }] = useMutation(CANCEL_MATCH_MUTATION, {
        onCompleted(data) {
            ...
            if (result.success) {
                ...
                setStatus('CANCELLED');
                setNotification({ message: 'Match request cancelled successfully.', status: 'CANCELLED' });
                setMatchResult(null);
            } else {
                console.error('Failed to cancel match request:', result.message);
                setNotification({ message: `Failed to cancel match request.`, status: 'ERROR' });
            }
        },
        onError(err) {
    ...
    useEffect(() => {
        ...
        const handleRequestExpiredEvent = (data: RequestExpiredData) => {
            if (status === 'QUEUED') {
                ...
                setMatchResult(null);
                setNotification({ message: 'Your match request has expired. Please try again.', status: 'REQUEST_EXPIRED' });
            }
        };
    ...
    return (
        <PageLayout header={<PageHeader title="No Clue" />}>
            <div className="bg-gray-100 w-full mx-auto rounded-xl p-8 shadow-xl">
                ...
                {notification && (
                    <div 
                        ...
                        className={`p-3 mb-6 rounded-lg text-sm font-medium flex items-center justify-between
                            ${getNotificationColorClasses(notification.status)}`
                        }
                    >
                        {/* Message Content */}
                ...
                {/* Matching Form */}
                <fieldset disabled={isFormDisabled} className={isFormDisabled ? 'opacity-60 transition-opacity' : ''}>
                    ...
                    <div className="mb-6">
                        ...
                        <select
                            ...
                            className={`w-full appearance-none border border-gray-300 bg-white rounded-xl py-3 px-4 shadow-md 
                                        ...
                                        ${isFormDisabled ? 'bg-gray-200 cursor-not-allowed' : ''}`}
                        >
                            {LANGUAGES.map(lang => (
